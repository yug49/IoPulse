...
User's current holding: ${userCurrentHolding || "None"}`,
        ...
        while (
            ...
            for (const toolCall of choice.message.tool_calls) {
                ...
                // Quantitative Analysis Agent requires real market data tools
                throw new Error(
                    `Quantitative Analysis Agent requires real market data tools. Cannot proceed without actual historical and current price data from ${toolCall.function.name} tool.`
                );
            }
        ...
        if (agentResponse) {
            try {
                ...
                if (!finalAnalysis) {
                    throw new Error(
                        `No JSON array found in quantitative analysis response: ${agentResponse.substring(
                            0,
                            500
                        )}...`
                    );
...
async function runQuantitativeAnalysisTests() {
    ...
    for (let i = 0; i < testCandidateSets.length; i++) {
        ...
        const result = await processQuantitativeAnalysis(
            test.candidates,
            test.userHolding
        );
